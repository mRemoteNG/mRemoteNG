...
namespace mRemoteNG.Tools
{
	public class PuttyTypeDetector
	{
		...
		private static bool IsKitty(string filename)
		{
			bool result = false;
			try {
				result = FileVersionInfo.GetVersionInfo(filename).InternalName.Contains("PuTTY") & FileVersionInfo.GetVersionInfo(filename).Comments.Contains("KiTTY");
			} catch {
				result = false;
			}
			return result;
		...
		private static bool IsXming(string filename)
		{
			bool result = false;
			try {
				result = FileVersionInfo.GetVersionInfo(filename).InternalName.Contains("PuTTY") & FileVersionInfo.GetVersionInfo(filename).ProductVersion.Contains("Xming");
			} catch {
				result = false;
			}
			return result;
