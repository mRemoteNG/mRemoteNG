	...
	public class Crypt
	{
		public static string Encrypt(string StrToEncrypt, string StrSecret)
		{
			...
			try {
				...
				mRemoteNG.App.Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, string.Format(mRemoteNG.My.Language.strErrorEncryptionFailed, ex.Message));
			}
		...
		public static string Decrypt(string ciphertextBase64, string password)
		{
			...
			try {
				...
				if (!ex is CryptographicException) {
					mRemoteNG.App.Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, string.Format(mRemoteNG.My.Language.strErrorDecryptionFailed, ex.Message));
				}
			}
