	...
	namespace Window
	{
		public partial class Tree : Base
		{
			...
			public void ApplyTheme()
			{
				...
				tvConnections.BackColor = _with1.ConnectionsPanelBackgroundColor;
				tvConnections.ForeColor = _with1.ConnectionsPanelTextColor;
				tvConnections.LineColor = _with1.ConnectionsPanelTreeLineColor;
				BackColor = _with1.ToolbarBackgroundColor;
				txtSearch.BackColor = _with1.SearchBoxBackgroundColor;
				txtSearch.ForeColor = _with1.SearchBoxTextPromptColor;
			}
			#endregion
			...
			{
				try {
					imgListTree.Images.Add(mRemoteNG.My.Resources.Root);
					imgListTree.Images.Add(mRemoteNG.My.Resources.Folder);
					imgListTree.Images.Add(mRemoteNG.My.Resources.Play);
					imgListTree.Images.Add(mRemoteNG.My.Resources.Pause);
					imgListTree.Images.Add(mRemoteNG.My.Resources.PuttySessions);
				} catch (Exception ex) {
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "FillImageList (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private void tvConnections_AfterLabelEdit(object sender, NodeLabelEditEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_AfterLabelEdit (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			{
				try {
					switch (mRemoteNG.Tree.Node.GetNodeType(e.Node)) {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_AfterSelect (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private void tvConnections_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
			{
				try {
					...
					if (e.Button == MouseButtons.Left) {
						if (Settings.SingleClickOnConnectionOpensIt & (mRemoteNG.Tree.Node.GetNodeType(e.Node) == mRemoteNG.Tree.Node.Type.Connection | mRemoteNG.Tree.Node.GetNodeType(e.Node) == mRemoteNG.Tree.Node.Type.PuttySession)) {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_NodeMouseClick (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private void tvConnections_MouseMove(object sender, MouseEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_MouseMove (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
				...
				try {
					...
					switch (mRemoteNG.Tree.Node.GetNodeType(selectedNode)) {
						case mRemoteNG.Tree.Node.Type.Connection:
							...
							if (!(connectionInfo.Protocol == mRemoteNG.Connection.Protocol.Protocols.SSH1 | connectionInfo.Protocol == mRemoteNG.Connection.Protocol.Protocols.SSH2)) {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "ShowHideTreeContextMenuItems (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			#endregion
			...
			private static void tvConnections_DragDrop(object sender, DragEventArgs e)
			{
				try {
					...
					if (object.ReferenceEquals(dropNode, targetNode)) {
						return;
					}
					...
					if (object.ReferenceEquals(dropNode, targetNode.Parent)) {
						return;
					}
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_DragDrop (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private static void tvConnections_DragEnter(object sender, DragEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_DragEnter (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private static void tvConnections_DragOver(object sender, DragEventArgs e)
			{
				try {
					...
					Root.PuttySessions.Info puttyRootInfo = null;
					while (!(targetNode == null)) {
						puttyRootInfo = targetNode.Tag as Root.PuttySessions.Info;
						if (puttyRootInfo != null | object.ReferenceEquals(targetNode, dropNode)) {
							e.Effect = DragDropEffects.None;
							return;
						}
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_DragOver (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private void tvConnections_ItemDrag(object sender, ItemDragEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_ItemDrag (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			#endregion

			#region "Tree Context Menu"
			private void cMenTreeAddConnection_Click(System.Object sender, EventArgs e)
			{
				AddConnection();
				Runtime.SaveConnectionsBG();
			}

			private void cMenTreeAddFolder_Click(System.Object sender, EventArgs e)
			{
				AddFolder();
				Runtime.SaveConnectionsBG();
			}

			private static void cMenTreeConnect_Click(System.Object sender, EventArgs e)
			{
				Runtime.OpenConnection(mRemoteNG.Connection.Info.Force.DoNotJump);
			}

			private static void cMenTreeConnectWithOptionsConnectToConsoleSession_Click(System.Object sender, EventArgs e)
			{
				Runtime.OpenConnection(mRemoteNG.Connection.Info.Force.UseConsoleSession | mRemoteNG.Connection.Info.Force.DoNotJump);
			}

			private static void cMenTreeConnectWithOptionsNoCredentials_Click(System.Object sender, EventArgs e)
			{
				Runtime.OpenConnection(mRemoteNG.Connection.Info.Force.NoCredentials);
			}

			private static void cMenTreeConnectWithOptionsDontConnectToConsoleSession_Click(System.Object sender, EventArgs e)
			{
				Runtime.OpenConnection(mRemoteNG.Connection.Info.Force.DontUseConsoleSession | mRemoteNG.Connection.Info.Force.DoNotJump);
			}

			private static void cMenTreeConnectWithOptionsConnectInFullscreen_Click(System.Object sender, EventArgs e)
			{
				Runtime.OpenConnection(mRemoteNG.Connection.Info.Force.Fullscreen | mRemoteNG.Connection.Info.Force.DoNotJump);
			}

			private static void cMenTreeConnectWithOptionsChoosePanelBeforeConnecting_Click(System.Object sender, EventArgs e)
			{
				Runtime.OpenConnection(mRemoteNG.Connection.Info.Force.OverridePanel | mRemoteNG.Connection.Info.Force.DoNotJump);
			}

			private void cMenTreeDisconnect_Click(System.Object sender, EventArgs e)
			{
				DisconnectConnection();
			}

			private static void cMenTreeToolsTransferFile_Click(System.Object sender, EventArgs e)
			{
				SshTransferFile();
			}

			private void mMenSortAscending_Click(System.Object sender, EventArgs e)
			{
			...
			private void cMenTreeToolsSortAscending_Click(System.Object sender, EventArgs e)
			{
				tvConnections.BeginUpdate();
			...
			private void cMenTreeToolsSortDescending_Click(System.Object sender, EventArgs e)
			{
				tvConnections.BeginUpdate();
			...
			private void cMenTreeDuplicate_Click(System.Object sender, EventArgs e)
			{
				mRemoteNG.Tree.Node.CloneNode(tvConnections.SelectedNode);
				Runtime.SaveConnectionsBG();
			}

			private static void cMenTreeRename_Click(System.Object sender, EventArgs e)
			{
				mRemoteNG.Tree.Node.StartRenameSelectedNode();
				Runtime.SaveConnectionsBG();
			}

			private static void cMenTreeDelete_Click(System.Object sender, EventArgs e)
			{
				mRemoteNG.Tree.Node.DeleteSelectedNode();
				Runtime.SaveConnectionsBG();
			}

			private static void cMenTreeImportFile_Click(System.Object sender, EventArgs e)
			{
				Import.ImportFromFile(Windows.treeForm.tvConnections.Nodes[0], Windows.treeForm.tvConnections.SelectedNode, true);
			}

			private static void cMenTreeImportActiveDirectory_Click(System.Object sender, EventArgs e)
			{
				Windows.Show(Type.ActiveDirectoryImport);
			}

			private static void cMenTreeImportPortScan_Click(System.Object sender, EventArgs e)
			{
				Windows.Show(mRemoteNG.UI.Window.Type.PortScan, true);
			}

			private static void cMenTreeExportFile_Click(System.Object sender, EventArgs e)
			{
				Export.ExportToFile(Windows.treeForm.tvConnections.Nodes[0], Windows.treeForm.tvConnections.SelectedNode);
			}
			private static void cMenTreeMoveUp_Click(System.Object sender, EventArgs e)
			{
				mRemoteNG.Tree.Node.MoveNodeUp();
				Runtime.SaveConnectionsBG();
			}

			private static void cMenTreeMoveDown_Click(System.Object sender, EventArgs e)
			{
			...
			public void AddConnection()
			{
				try {
					...
					if (newTreeNode == null) {
						Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "UI.Window.Tree.AddConnection() failed." + Constants.vbNewLine + "mRemoteNG.Tree.Node.AddNode() returned Nothing.", true);
						return;
					}
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "UI.Window.Tree.AddConnection() failed." + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			public void AddFolder()
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, string.Format(Language.strErrorAddFolderFailed, ex.Message), true);
				}
			}
			...
			private void DisconnectConnection()
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "DisconnectConnection (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private static void SshTransferFile()
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "SSHTransferFile (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private void AddExternalApps()
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "cMenTreeTools_DropDownOpening failed (UI.Window.Tree)" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private static void StartExternalApp(Tools.ExternalTool externalTool)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "cMenTreeToolsExternalAppsEntry_Click failed (UI.Window.Tree)" + Constants.vbNewLine + ex.Message, true);
				}
			}
			#endregion

			#region "Menu"
			private static void mMenViewExpandAllFolders_Click(System.Object sender, EventArgs e)
			{
				mRemoteNG.Tree.Node.ExpandAllNodes();
			}

			private void mMenViewCollapseAllFolders_Click(System.Object sender, EventArgs e)
			{
				if (tvConnections.SelectedNode != null) {
			...
			{
				txtSearch.ForeColor = mRemoteNG.Themes.ThemeManager.ActiveTheme.SearchBoxTextColor;
			...
			private void txtSearch_LostFocus(object sender, EventArgs e)
			{
				if (string.IsNullOrEmpty(txtSearch.Text)) {
					txtSearch.ForeColor = mRemoteNG.Themes.ThemeManager.ActiveTheme.SearchBoxTextPromptColor;
					txtSearch.Text = Language.strSearchPrompt;
				}
			}
			...
			private void txtSearch_KeyDown(object sender, KeyEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "txtSearch_KeyDown (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}

			private void txtSearch_TextChanged(System.Object sender, EventArgs e)
			{
			...
			private void tvConnections_KeyPress(object sender, KeyPressEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_KeyPress (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			...
			private void tvConnections_KeyDown(object sender, KeyEventArgs e)
			{
				try {
					...
					Runtime.MessageCollector.AddMessage(mRemoteNG.Messages.MessageClass.ErrorMsg, "tvConnections_KeyDown (UI.Window.Tree) failed" + Constants.vbNewLine + ex.Message, true);
				}
			}
			#endregion
		}
