...
namespace mRemoteNG.Config.Putty
{
	public class Sessions
	{
			...
			foreach (Provider provider in Providers) {
				...
				foreach (Connection.PuttySession.Info sessionInfo in savedSessions) {
					...
					if (rootTreeNode.Nodes.ContainsKey(sessionInfo.Name)) {
						...
						treeNode = mRemoteNG.Tree.Node.AddNode(mRemoteNG.Tree.Node.Type.PuttySession, sessionInfo.Name);
						if (treeNode == null)
							continue;
				...
				if (inUpdate) {
					mRemoteNG.Tree.Node.Sort(rootTreeNode, SortOrder.Ascending);
					rootTreeNode.Expand();
					treeView.EndUpdate();
				}
			}
		...
		{
			bool enabled = true;
			switch (PuttyTypeDetector.GetPuttyType()) {
		...
		public class SessionList : StringConverter
		{
			...
			public static string[] Names {
				get { return Sessions.GetSessionNames(); }
			}
