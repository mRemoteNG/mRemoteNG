...
namespace mRemoteNG.Config.Import
{
	public class RemoteDesktopConnectionManager
	{
		public static void Import(string fileName, TreeNode parentTreeNode)
		{
			...
			int schemaVersion = rdcManNode.Attributes["schemaVersion"].Value;
			if (!(schemaVersion == 1)) {
		...
		private static Connection.Info ConnectionInfoFromXml(XmlNode xmlNode)
		{
			...
			if (remoteDesktopNode.Attributes["inherit"].Value == "None") {
				string resolutionString = remoteDesktopNode.SelectSingleNode("./size").InnerText.Replace(" ", "");
				try {
					connectionInfo.Resolution = "Res" + mRemoteNG.Tools.Misc.StringToEnum(typeof(Connection.Protocol.RDP.RDPResolutions), resolutionString);
		...
		private static string DecryptPassword(string ciphertext)
		{
			...
			Win32.DATA_BLOB plaintextData = default(Win32.DATA_BLOB);
			try {
				...
				Win32.DATA_BLOB ciphertextData = default(Win32.DATA_BLOB);
				ciphertextData.cbData = ciphertextArray.Length;
				ciphertextData.pbData = gcHandle.AddrOfPinnedObject();

				if (!Win32.CryptUnprotectData(ref ciphertextData, null, ref null, null, ref null, 0, ref plaintextData))
					return null;

				int plaintextLength = plaintextData.cbData / 2;
				// Char = 2 bytes
				char[] plaintextArray = new char[plaintextLength];
				Marshal.Copy(plaintextData.pbData, plaintextArray, 0, plaintextLength);

				return new string(plaintextArray);
			} catch (Exception ex) {
				mRemoteNG.App.Runtime.MessageCollector.AddExceptionMessage("RemoteDesktopConnectionManager.DecryptPassword() failed.", ex, , true);
				return null;
			} finally {
