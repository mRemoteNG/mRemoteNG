...
public sealed class MultiAddressPlugin : IConnectionPropertyProviderPlugin, IConnectionAddressResolverPlugin
{
    ...
    public async Task ResolveAsync(IPluginConnection connection, CancellationToken cancellationToken)
    {
        ...
        bool isValidHostname = IsValidHostname(hostname);
        bool isValidIpAddress = IPAddress.TryParse(ipAddress, out IPAddress? configuredIpAddress) && configuredIpAddress is not null;
        ...
        if (!isValidHostname)
        {
            ...
            _context?.Messages.Warning(string.Format(GetString("MultiAddressInvalidSavedHostnameNoFallback", "Multi-address plugin: '{0}' has an invalid saved hostname and no valid IP address fallback is available."), connection.Name));
            connection.Hostname = resolvedTarget;
            return;
        }
        ...
        {
            bool hostnameWasPreferredTarget = string.Equals(resolvedTarget, hostname, StringComparison.Ordinal);
            string resourceName = hostnameWasPreferredTarget ? "MultiAddressHostnameNotResolvedUsingIp" : "MultiAddressHostnameNotResolved";
            string fallback = hostnameWasPreferredTarget
                ? "Multi-address plugin: the saved hostname for '{0}' could not be resolved. Using the saved IP address instead."
                : "Multi-address plugin: the saved hostname for '{0}' could not be resolved.";
            _context?.Messages.Warning(string.Format(GetString(resourceName, fallback), connection.Name));
            connection.Hostname = hostnameWasPreferredTarget ? ipAddress : resolvedTarget;
            return;
        }
