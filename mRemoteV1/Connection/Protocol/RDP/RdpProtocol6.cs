...
*/
            ...
            try
            {
                ...
                if (connectionInfo.RDGatewayUsageMethod != RDGatewayUsageMethod.Never)
                {
                    ...
                    if (_rdpVersion >= Versions.RDC61 && !Force.HasFlag(ConnectionInfo.Force.NoCredentials))
                    {
                        ...
                        {
                            var userName = GetUserName(connectionInfo?.Username ?? "");
                            var domain = GetDomain(connectionInfo?.Domain ?? "");

                            _rdpClient.TransportSettings2.GatewayUsername = userName;
                            _rdpClient.TransportSettings2.GatewayDomain = domain;
                            _rdpClient.TransportSettings2.GatewayPassword = GetPassword((connectionInfo?.Password ?? ""), userName, domain, connectionInfo.Hostname);
                        }
                        ...
                        {
                            var userName = GetUserName(connectionInfo.RDGatewayUsername);
                            var domain = GetDomain(connectionInfo.RDGatewayDomain);

                            _rdpClient.TransportSettings2.GatewayUsername = userName;
                            _rdpClient.TransportSettings2.GatewayDomain = domain;
                            _rdpClient.TransportSettings2.GatewayPassword = GetPassword(connectionInfo.RDGatewayPassword, userName, domain, connectionInfo.Hostname);
                            _rdpClient.TransportSettings2.GatewayCredSharing = 0;
                        }
                    }
                }
        ...
        private void SetCredentials()
        {
            try
            {
                ...
                _rdpClient.AdvancedSettings2.ClearTextPassword = GetPassword((connectionInfo?.Password ?? ""), userName, domain, connectionInfo.Hostname);
