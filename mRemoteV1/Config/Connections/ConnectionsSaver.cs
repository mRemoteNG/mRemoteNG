...
namespace mRemoteNG.Config.Connections
{
    public class ConnectionsSaver
    {
            ...
            try
            {
                ...
                if (databaseVersion.CompareTo(new Version(2, 2)) == 0) // 2.2
                {
                    Runtime.MessageCollector.AddMessage(MessageClass.InformationMsg,
                        $"Upgrading database from version {databaseVersion} to version 2.3.");
                    sqlCommand =
                        new SqlCommand(
                ...
                {
                    Runtime.MessageCollector.AddMessage(MessageClass.InformationMsg,
                        $"Upgrading database from version {databaseVersion} to version 2.4.");
                    sqlCommand =
                        new SqlCommand(
                ...
                {
                    Runtime.MessageCollector.AddMessage(MessageClass.InformationMsg,
                        $"Upgrading database from version {databaseVersion} to version 2.5.");
                    sqlCommand =
                        new SqlCommand(
        ...
        private void UpdateUpdatesTable(SqlDatabaseConnector sqlDatabaseConnector)
        {
            ...
            sqlQuery.ExecuteNonQuery();
            sqlQuery =
                new SqlCommand("INSERT INTO tblUpdate (LastUpdate) VALUES(\'" + MiscTools.DBDate(DateTime.Now) + "\')",
                    sqlDatabaseConnector.SqlConnection);
            sqlQuery.ExecuteNonQuery();
        }
