...
namespace mRemoteNG.Config.Putty
{
    public class PuttySessionsXmingProvider : AbstractPuttySessionsProvider
    {
        ...
        public override RootPuttySessionsNodeInfo RootInfo { get; } = new RootPuttySessionsNodeInfo
        {
            Name = "Xming Putty Sessions"
        };
        ...
        public override string[] GetSessionNames(bool raw = false)
        {
            ...
            foreach (var sessionName in Directory.GetFiles(sessionsFolderPath))
            {
                ...
                sessionNames.Add(raw ? sessionFileName : HttpUtility.UrlDecode(sessionFileName?.Replace("+", "%2B")));
            }
            ...
            var registrySessionNames =
                PuttySessionsRegistryProvider.GetSessionNames(raw)
                    .Select(sessionName => string.Format(RegistrySessionNameFormat, sessionName))
                    .ToList();
        ...
        {
            var puttyPath = "";
            puttyPath = mRemoteNG.Settings.Default.UseCustomPuttyPath
                ? Convert.ToString(mRemoteNG.Settings.Default.CustomPuttyPath)
