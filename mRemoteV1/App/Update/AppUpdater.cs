...
namespace mRemoteNG.App.Update
{
    public class AppUpdater
    {
        ...
        public void SetProxySettings(bool useProxy, string address, int port, bool useAuthentication, string username,
            string password)
        {
        ...
        public void DownloadUpdateAsync()
        {
            ...
            CurrentUpdateInfo.UpdateFilePath = Path.Combine(Path.GetTempPath(),
                Path.ChangeExtension(Path.GetRandomFileName(), "exe"));
        ...
        {
            var updateFileUri = new Uri(new Uri(Convert.ToString(Settings.Default.UpdateAddress)),
                new Uri(UpdateChannelInfo.FileName, UriKind.Relative));
        ...
        public event DownloadProgressChangedEventHandler DownloadUpdateProgressChangedEvent
        {
            add
            {
                DownloadUpdateProgressChangedEventEvent =
                    ...
                    Delegate.Combine(DownloadUpdateProgressChangedEventEvent, value);
            }
            remove
            {
                DownloadUpdateProgressChangedEventEvent =
                    (DownloadProgressChangedEventHandler)
                    Delegate.Remove(DownloadUpdateProgressChangedEventEvent, value);
            }
        }
