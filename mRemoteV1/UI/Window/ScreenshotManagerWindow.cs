...
namespace mRemoteNG.UI.Window
{
    public class ScreenshotManagerWindow : BaseWindow
    {
        ...
        public void AddScreenshot(Image Screenshot)
        {
            ...
            catch (Exception ex)
            {
                Runtime.MessageCollector.AddMessage(MessageClass.ErrorMsg,
                    "AddScreenshot (UI.Window.ScreenshotManager) failed" + Environment.NewLine + ex.Message, true);
            }
        }
        ...
        private void OpenScreenshot(PictureBox sender)
        {
            ...
            {
                Runtime.MessageCollector.AddMessage(MessageClass.ErrorMsg,
                    "OpenScreenshot (UI.Window.ScreenshotManager) failed" + Environment.NewLine + ex.Message, true);
            }
        }
        ...
        private void btnCloseScreenshot_Click(object sender, EventArgs e)
        {
            ...
            {
                Runtime.MessageCollector.AddMessage(MessageClass.ErrorMsg,
                    "btnCloseScreenshot_Click (UI.Window.ScreenshotManager) failed" + Environment.NewLine + ex.Message,
                    true);
            }
        }
        ...
        private void SaveAllImages()
        {
            try
            {
                ...
                if (dlgSaveAllImages.ShowDialog() == DialogResult.OK)
                {
                    foreach (
                        var fPath in
                        Directory.GetFiles(dlgSaveAllImages.SelectedPath, "Screenshot_*", SearchOption.TopDirectoryOnly)
                    )
                    {
                    ...
                    foreach (Control ctrl in flpScreenshots.Controls)
                        if (ctrl is PictureBox)
                        {
                            (ctrl as PictureBox).Image.Save(
                                ...
                                MiscTools.LeadingZero(Convert.ToString(pCount)) + ".png", ImageFormat.Png);
                            pCount++;
                        }
                }
            }
            ...
            {
                Runtime.MessageCollector.AddMessage(MessageClass.ErrorMsg,
                    "SaveAllImages (UI.Window.ScreenshotManager) failed" + Environment.NewLine + ex.Message, true);
            }
        }
        ...
        private void CopyImageToClipboard()
        {
            ...
            {
                Runtime.MessageCollector.AddMessage(MessageClass.ErrorMsg,
                    "CopyImageToClipboard (UI.Window.ScreenshotManager) failed" + Environment.NewLine + ex.Message, true);
            }
        }
        ...
        private void SaveSingleImage()
        {
            try
            {
                if (dlgSaveSingleImage.ShowDialog() == DialogResult.OK)
                    switch (
                        dlgSaveSingleImage.FileName.Substring(
                            dlgSaveSingleImage.FileName.LastIndexOf(".", StringComparison.Ordinal) + 1).ToLower())
                    {
                        case "gif":
                            ((PictureBox) cMenScreenshot.Tag).Image.Save(dlgSaveSingleImage.FileName, ImageFormat.Gif);
                            break;
                        case "jpeg":
                            ((PictureBox) cMenScreenshot.Tag).Image.Save(dlgSaveSingleImage.FileName, ImageFormat.Jpeg);
                            break;
                        case "jpg":
                            ((PictureBox) cMenScreenshot.Tag).Image.Save(dlgSaveSingleImage.FileName, ImageFormat.Jpeg);
                            break;
                        case "png":
                            ((PictureBox) cMenScreenshot.Tag).Image.Save(dlgSaveSingleImage.FileName, ImageFormat.Png);
                            break;
                    }
            }
            catch (Exception ex)
            {
                Runtime.MessageCollector.AddMessage(MessageClass.ErrorMsg,
                    "SaveSingleImage (UI.Window.ScreenshotManager) failed" + Environment.NewLine + ex.Message, true);
            }
        }
